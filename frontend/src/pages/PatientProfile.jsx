...
const PatientDashboard = () => {
    ...
    useEffect(() => {
        ...
        if (session) {
            ...
            const activeUserId = sessionData.userId || sessionData.id;
            const activePatientId = sessionData.patientId; // Extracting true Patient Profile Key (e.g., 17)

            if (activeUserId && activePatientId) {
                setUserId(activeUserId);
                setPatientId(activePatientId);
                
                // Prefill fallback parameters safely using initial session payload strings if available
    ...
    // Fetch patient records dynamically from the backend patient database segment
    useEffect(() => {
        if (!patientId) return;

        const fetchPatientData = async () => {
            try {
                //  Direct explicit target routing matching your new endpoint pathing
        ...
        fetchPatientData();
    }, [patientId]); //  Triggers immediately upon parsing true patientId from storage
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            const unifiedSession = { 
                id: userId,            // Keeps alignment for core filters
                userId: userId,        // Authentication Key
                patientId: patientId,  // Profile Record Row Primary Key
                ...data 
            };
            ...
            <div className="flex min-h-screen items-center justify-center bg-slate-50">
                <div className="max-w-md w-full bg-white p-6 rounded-xl shadow-sm border border-slate-200 text-center">
                    ...
                    <p className="text-xs text-slate-500 mt-1 mb-4">
                        No synchronized patient data matches system locator reference key ID: {patientId || userId}.
                    </p>
                    ...
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            ...
                            <h1 className="text-lg font-black tracking-tight text-slate-800">
                                HealthNode <span className="text-blue-600 font-medium text-base">Medical Records</span>
                            </h1>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
                        <button onClick={() => navigate("/book-appointment")} className="text-xs font-bold uppercase tracking-wider bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg transition duration-150">
                            Book Appointment
                        </button>
                        <button onClick={handleLogout} className="text-xs font-bold uppercase tracking-wider bg-slate-50 hover:bg-red-50 text-slate-600 hover:text-red-600 border border-slate-200 px-3 py-1.5 rounded-lg transition duration-150">
                            Sign Out
                        </button>
                    </div>
                </div>
                        ...
                        <div className="bg-white border border-slate-200/70 rounded-2xl p-6 shadow-sm shadow-slate-100 sticky lg:top-24">
                            <div className="flex flex-col items-center text-center pb-6 border-b border-slate-100">
                                ...
                                <span className="text-xs font-medium text-slate-400 mt-0.5">
                                    Patient EHR Identifier: #{patientId || "Pending"}
                                </span>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            <div className={`bg-white border rounded-2xl shadow-sm transition-all overflow-hidden ${patient.knownDrugAllergies ? "border-red-200/80 shadow-red-50/50" : "border-slate-200/70"}`}>
                                <div className={`px-5 py-4 flex items-center gap-3 border-b ${patient.knownDrugAllergies ? "bg-red-50/30 border-red-100" : "bg-slate-50/50 border-slate-100"}`}>
                                    ...
                                    <h3 className={`text-xs font-bold uppercase tracking-wider ${patient.knownDrugAllergies ? "text-red-800" : "text-slate-500"}`}>
                                ...
                                <div className="p-5">
                                    <p className={`text-sm leading-relaxed whitespace-pre-line font-medium ${patient.knownDrugAllergies ? "text-slate-800" : "text-slate-400 italic"}`}>
                                        {patient.knownDrugAllergies || "No known clinical pharmaceutical or compound responses recorded."}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-slate-200/70 rounded-2xl shadow-sm overflow-hidden">
                                ...
                                <div className="p-5">
                                    <p className={`text-sm leading-relaxed whitespace-pre-line font-medium ${patient.chronicConditions ? "text-slate-800" : "text-slate-400 italic"}`}>
                                        {patient.chronicConditions || "No active long-term pathogenetic mappings or chronic constraints recorded."}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-slate-200/70 rounded-2xl shadow-sm overflow-hidden">
                                ...
                                <div className="p-5">
                                    <p className={`text-sm font-semibold tracking-tight ${patient.emergencyContactDetails ? "text-slate-800" : "text-slate-400 italic"}`}>
                                        {patient.emergencyContactDetails || "No priority designated proxy contact array configured."}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSave} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="bloodType" placeholder="e.g., O+, A-" value={formData.bloodType} onChange={handleChange} className="w-full text-sm p-2.5 border border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition bg-slate-50/40 font-medium" />
                                </div>
                            </div>
